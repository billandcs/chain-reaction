...
export default async function AlertsPage() {
  const [rules, events] = await Promise.all([
    ...
    prisma.alertEvent.findMany({ include: { rule: true }, orderBy: { createdAt: "desc" }, take: 20 }),
  ]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <MetricTile label="Rules" value={String(rules.length)} detail={`${enabledRules} enabled`} icon={ShieldCheck} tone="blue" />
        <MetricTile label="Events" value={String(events.length)} detail="Recent local history" icon={BellRing} tone="amber" />
        <MetricTile label="Unread" value={String(unreadEvents)} detail="Notification center" icon={Bell} tone="rose" />
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Panel title="Rules" eyebrow="Evaluation engine">
          <div className="divide-y divide-[#e2e6e1] dark:divide-[#25302b]">
            {rules.length === 0 ? (
              <div className="py-8 text-sm text-[#64706b] dark:text-[#9aa39e]">No alert rules configured.</div>
            ) : (
              rules.map((rule) => (
                <div key={rule.id} className="flex flex-col justify-between gap-3 py-3 sm:flex-row sm:items-center">
        ...
        <Panel title="Events" eyebrow="Triggered alerts">
          <div className="divide-y divide-[#e2e6e1] dark:divide-[#25302b]">
            {events.length === 0 ? (
              <div className="py-8 text-sm text-[#64706b] dark:text-[#9aa39e]">No events yet.</div>
            ) : (
              events.map((event) => (
                <div key={event.id} className="py-3">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-sm leading-6 text-[#64706b] dark:text-[#9aa39e]">{event.message}</div>
                    </div>
